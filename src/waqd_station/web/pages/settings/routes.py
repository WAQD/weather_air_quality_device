...
async def trigger_update():
    try:
        assert (
            latest_release := app.comp_ctrl().components.auto_updater.latest_release
        ) is not None
        app.comp_ctrl().components.auto_updater.install_update(latest_release.tag_name)
...
async def system_update():
    try:
        app.comp_ctrl().components.auto_updater.system_gui_update()
...
async def about():
    content = sub_template(
        "about.html",
        {"version": waqd_station.__version__, "platform": RuntimeSystem().platform},
        current_path,
    )
    return content
